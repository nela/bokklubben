...
<div data-testid="login-component" aria-label="Lo In" class="login-form">
  <form data-testid="email-pass-form" on:submit={handler.emailAndPassword(email, password)}>
	...
	<input aria-label="Password" type="password" placeholder="Password" bind:value={password} required />
	<button
		type="submit"
